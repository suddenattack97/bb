...
실행: uvicorn server_v3:app --host 0.0.0.0 --port 8002
"""
import asyncio
import base64
import io
...
LOG_FILE = LOG_DIR / "server_v3.txt"
...
POLL_INTERVAL_S = 5
FUTURES_POLL_INTERVAL_S = 60
BINANCE_SYMBOL = "BTC/USDT"
MAX_CANDLE_BUFFER = 60
PRED_STEPS = 5
SNAPSHOT_DIR = Path("snapshots")
CANDLES_DIR = Path("data/candles")
PREDICTIONS_DIR = Path("data/predictions_v3")
...
def _save_prediction(origin_time: int, predictions: list, last_close: float, direction: int, confidence: float) -> bool:
    ...
    try:
        ...
        print(f"[V3 SAVE] 예측 저장 실패: {e}")
        return False


app = FastAPI(title="BTC V3 방향성 예측 서버")
...
def _fetch_futures_data_sync():
    ...
    try:
        ...
        if _futures_oi == 0 and _futures_oi_prev == 0:
            _futures_oi = _futures_oi_prev = oi_val
        else:
...
def _load_model():
    ...
    if not TORCH_AVAILABLE:
        print("[V3] PyTorch 없음 → fallback")
        return
    try:
        import torch
        mp, sp = Path("tcn_v3_model.pth"), Path("scaler_v3.npy")
        if not mp.exists() or not sp.exists():
            print("[V3] tcn_v3_model.pth / scaler_v3.npy 없음 → python train_initial_v3.py 실행 필요")
            return
        sc = np.load(str(sp), allow_pickle=True).item()
        nf = int(sc.get("num_features", 11))
        _feature_names = sc.get("feature_names", [])
        _use_oi_funding = sc.get("use_oi_funding", False)
        ...
        _scaler_std = sc["std"]
        print(f"[V3] TCNClassifier 로드 완료 (피처 {nf}개)")
    except Exception as e:
        print(f"[V3] 로드 실패: {e}")
...
def _run_prediction(buf: list) -> Optional[list]:
    ...
    if _model is not None:
        try:
            ...
            _last_pred_debug = {
                "model": "TCNClassifier_v3",
                "direction": direction,
                "probs": [float(p) for p in probs],
                "origin_time": last_ts,
            }
        except Exception as e:
            print(f"[V3 PRED] 오류: {e}")
...
async def _startup():
    ...
    asyncio.create_task(_futures_poll_loop())
...
async def status():
    return {
        ...
        "model": "TCNClassifier_v3" if _model else "fallback",
...
async def download_logs():
    ...
    return FileResponse(LOG_FILE, filename="server_v3.txt", media_type="text/plain")
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("server_v3:app", host="0.0.0.0", port=8002, reload=False)
